...
flowchart LR
    ...
    subgraph OFFICE["② LegalOffice / Word Editor 服务"]
        NGINX["LegalOffice Nginx<br/>legaloffice.lenovo.com"]
        SESSION["编辑会话接口<br/>POST /api/integrations/legalai/session"]
        EDITOR["Word Editor<br/>DOCX解析与在线编辑"]
        DRAFT[("Word Editor草稿存储<br/>HTML + DOCX + rev")]
...
3. 保存逻辑
   ...
   - 内部自动保存：保存 Word Editor 草稿，不调用 LegalAI，不回写 S3。
...
6. 当前功能边界
   ...
   - word-editor 可以保留内部历史快照；当前 LegalAI 测试页通过 `history: false` 隐藏历史按钮。
...
8. 可通过审查历史页的word-editor集成测试按钮进行测试。
...
10. REST 接口与 LegalAI 接入的关系
   ...
   REST 不是另一种业务系统集成方案，而是 word-editor 暴露编辑能力的一种 HTTP 调用方式。例如文档列表、内容读取、页面设置和历史版本都可以通过 REST 操作。当前 LegalAI 页面主要使用 iframe SDK；SDK 内部仍会通过 REST 保存草稿、读取文档和正式提交。

   - `POST /api/documents/import`：供 word-editor 独立页面导入本地 `.docx/.doc` 文件；当前 LegalAI 链路不调用。
